...
from openseize.spectra import metrics as _os_metrics

# ------------------------------------------------------- PSD backend (openseize)
#
# This module is the openseize port of `scipy_backend.py`. Only the PSD
# estimation and (optionally) the band integration differ; every band
# definition, tier rule, mains suppression and derived feature is unchanged.
#
# `openseize.spectra.estimators.psd` is a Welch estimator. Called with
# `resolution = sfreq / nperseg` on a window whose length equals nperseg there is
# exactly one segment, so it reproduces `scipy.signal.welch(x, nperseg=len(x))`
# to floating-point noise (verified: max |dPSD| ~2e-16, per-epoch rows
# independent). That is why this port does not change the model's numbers.
#
# Band integration is a real choice, however:
#   "trapezoid" -- np.trapezoid, what the published model was fitted with.
#   "simpson"   -- openseize.spectra.metrics.power, which uses Simpson's rule.
# On noisy single-window PSDs with sharp peaks these differ materially (~10% on
# a delta band in testing), so the default stays trapezoid and switching is an
# explicit, documented decision rather than an accident of porting.
INTEGRATION = "trapezoid"


def _welch(x: np.ndarray, sfreq: float, nperseg: int,
           axis: int = -1) -> tuple[np.ndarray, np.ndarray]:
    """Welch PSD via openseize, matching scipy.signal.welch(nperseg=nperseg).
...
def _band_power(freqs: np.ndarray, psd: np.ndarray,
    ...
    if INTEGRATION == "simpson":
        # openseize-native: Simpson's rule over the inclusive band slice.
        sub = psd[:, idx] if psd.ndim == 2 else psd[idx]
        val = _os_metrics.power(sub, freqs[idx], axis=-1)
        return val if psd.ndim == 2 else float(val)
    if INTEGRATION != "trapezoid":
        raise ValueError(f"INTEGRATION must be 'trapezoid' or 'simpson', "
                         f"got {INTEGRATION!r}")
